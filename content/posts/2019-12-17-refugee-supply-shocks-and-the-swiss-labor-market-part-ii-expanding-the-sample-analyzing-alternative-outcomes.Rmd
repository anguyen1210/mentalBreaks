...
In my original analysis, I use a fairly restricted sample of prime-age working males, mimicking the original Borjas approach. It has been argued by many others, notably [Clemens in this Vox article](https://www.vox.com/the-big-idea/2017/6/23/15855342/immigrants-wages-trump-economics-mariel-boatlift-hispanic-cuban), and even more recently by Banarjee and Duflo[^4], that while the data is very noisy, there are not necessarily any good *economic* reasons from excluding non-prime age workers and women from the sample.  
  
With that in mind, in this follow-up post, I wanted to see what happened to our estimates of the impact of the Balkan refugees on Swiss employment once I expanded the sample to include all working age adults, both male and female. Furthermore, as our data includes variables that capture other elements of the labor market--namely, contract types and labor force participation--so I have a look to see if the Balkan shock has had any impact on these outcomes as well. Given that my original analysis of prime working-age males found no significant effect of the Balkan refugees on the Swiss unemployment rate, I anticipate largely the same results in this follow-up.

## Empirical specification

The specification for this analysis has not changed much since the first post, other than I add a control for women, given our expanded sample. We, again, use the region-skill cell approach, which has the advantage of being a national-level approach that accounts for the uneven distribution of immigrants across different local labor markets, along the movement of native workers (i.e. factor flows) as they adjust in response and re-balance the national market.  
    
Practically, this approach entails dividing the national labor market into a number of different "cells" that are defined by a regional unit and unit capturing the skill levels of the workers. Given sufficient data, the skills can be refined to capture $N$ dimensions (e.g. age groups, occupation types, etc.).  
...
As mentioned above, I wanted to see how much the results were affected by the sample restrictions, so for this analysis, I created three different sub-samples of the Swiss census data: (1) prime-working age males only (Ages 15-54), (2) all working age males (Ages 15+), and all working age male and females (Ages 15+). In keeping with the literature, I use educational attainment level to proxy for worker skill-level, with the idea that workers with similar skill-levels in the same regional markets would compete for jobs.  

When we look at the educational attainment distribution for native workers, we can see that the majority of the sample has completed secondary education, and there is a slightly higher number of those with post-secondary education as opposed to only primary education. Glancing at the *Figure 1* below, we can see that, no matter how we slice the sample, the relative proportions of this educational attainment distribution remains the same across all samples.  
...
On the other hand, for the Balkan population, the picture changes considerably, depending on which sample we use. As can be seen in the *Figure 2*, when we restrict our sample to only prime-working age males, the bulk of the population has only primary education or less, but as we relax this sample restriction the educational attainment distribution changes considerably. In our full sample of all working age male and females, the bulk of the Balkan population has  completed secondary education, similar to the native workforce, though the split between secondary and primary education for the Balkan workers is a bit closer than that found for natives in the same sample.  
...
For the rest of this analysis, I will only be reporting the results using the full sample of male and females of working age, though it should be noted that the regression results using all three samples had the same signs and significance levels regardless of which sample was used.  
...
With regards to where these shocks were felt the most, we can see that, while the variation is modest, not all of the cantons in Switzerland were affected the same way, as we would expect. In the map below, we can see that majority of of cantons only experienced relatively light "shocks” of 1 to 2%, and the biggest impact biggest impacts tend to have occurred in the central and northeastern parts of the country.
...
When we do a simple scatter plot of showing the relationship between any of the labor market outcome variables and the size of the Balkan shock, we notice that (a) there appears to be some sort of linear relationship between the two variables, (b) the direction, or sign, of this relationship is as we predicted from before, and that (c) the impact of the Balkan shock only seems to affect small cells containing workers with the lowest level of education.  
